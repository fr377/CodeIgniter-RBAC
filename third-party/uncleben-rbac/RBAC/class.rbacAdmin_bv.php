...
/**
  ...
  *
  * Database connection uses the db_bv class. (You need the class.bv_db.php file)
...
class RbacAdmin_bv
{
	...
	var $mConn; // db_bv database connection
	var $mLog; // Log some information
	
	/**
	  * CONSTRUCTOR
	  **/
	  function RbacAdmin_bv($conn){
		  $this->mConn = $conn;
		  $this->mLog = '';
	  }
	...
	/**
	  ...
	  **/
	function SetPrivilege($name, $desc, $arrActions = '')
	{
		$conn = $this->mConn;
		...
		if ($_row = $conn->GetRow($_sql)){
			...
			$this->mLog .= "Privilege <b>$name</b> with id <b>$privilege_id</b> already exists.<br>";
		}
		else {
			...
			$this->mLog .= "New privilege inserted <b>$name</b> with id <b>$privilege_id</b><br>";
		}
		...
		foreach ($arrActions as $_action=>$_desc){
			...
			if (!$_row = $conn->GetRow($_sql)){ // action does not exist
				...
				$this->mLog .= "New action inserted <b>$_action</b> with id <b>$action_id</b><br>";
			}
			else{
				$action_id = $_row['id'];
				$this->mLog .= "Action <b>$_action</b> with id <b>$action_id</b> already exists.<br>";
			}
			...
			if ($is_new_privilege || $is_new_action){
				...
				if ($conn->Insert()){
					$this->mLog .= "New relationship between <b>$name ($privilege_id)</b> and <b>$_action ($action_id)</b> has been added.<br>";
				}
			}
			...
			if ($is_new_action){
				...
				$this->mLog .= "New singular privilege inserted <b>$_action</b> with id <b>$action_privilege_id</b><br>";
				...
				$this->mLog .= "New singluar relationship between <b>$_action ($action_privilege_id)</b> and <b>$_action ($action_id)</b> has been added.<br>";
	...
	/**
	  ...
	  **/
	function SetDomain($name, $desc, $arrObjects = '')
	{
		$conn = $this->mConn;
		...
		if ($_row = $conn->GetRow($_sql)){
			$domain_id = $_row['id'];
			$this->mLog .= "Domain <b>$name</b> with id <b>$domain_id</b> already exists.<br>";
		}
		else {
			...
			$this->mLog .= "New domain inserted <b>$name</b> with id <b>$domain_id</b><br>";
		}
		...
		foreach ($arrObjects as $_object=>$_desc){
			...
			if (!$_row = $conn->GetRow($_sql)){ // object does not exist
				...
				$this->mLog .= "New object inserted <b>$_object</b> with id <b>$object_id</b><br>";
			}
			else{
				$object_id = $_row['id'];
				$this->mLog .= "Object <b>$_object</b> with id <b>$object_id</b> already exists.<br>";
			}
			...
			if ($is_new_domain || $is_new_object){
				...
				if ($conn->Insert()){
					$this->mLog .= "New relationship between <b>$name ($domain_id)</b> and <b>$_object ($object_id)</b> has been added.<br>";
				}
			}
			...
			if ($is_new_object){
				...
				$this->mLog .= "New singular domain inserted <b>$_object</b> with id <b>$object_domain_id</b><br>";
				...
				$this->mLog .= "New singluar relationship between <b>$_object ($object_domain_id)</b> and <b>$_object ($object_id)</b> has been added.<br>";
	...
	/**
	  ...
	  *
	  **/
	function SetRole($name, $importance = 0, $arrDefs, $description='')
	{
		$conn = $this->mConn;
		...
		if ($_row = $conn->GetRow($_sql)){
			$role_id = $_row['id'];
			$this->mLog .= "Role <b>$name</b> with id <b>$role_id</b> already exists.<br>";
		}
		else {
			// Create the role
			...
			$role_id = $conn->GetId();
			$this->mLog .= "New role: <b>$name</b> created.<br>";
		}
		...
		if ($role_id){
			...
			foreach ($arrDefs as $arr_def){
				...
				if ($privilege_id && $domain_id){ // If we have a valid ID
					...
					$this->mLog .= "-- Role <b>$name</b> with importance $importance, has the following privilege (<b>$_privilege: $privilege_id</b>) and domain (<b>$_domain: $domain_id</b>). Is allowed: <b>$is_allowed</b><br>";
				}
			}
		}
	}
	...
	function SetUserRole($userId, $role)
	{
		...
		if (!$role_id = $this->GetRowId('rbac_roles', $role))
		{
			$this->mLog .= "The role<b>$role</b> does not exist.<br>";
			return FALSE;
		}
		...
		if ($conn->Insert()){
			$this->mLog .= "User <b>$userId</b> has been given the role <b>$role</b>.<br>";
