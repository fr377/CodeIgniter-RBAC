...
class rbac_users_has_roles_view extends securePage_bv
{

	Function rbac_users_has_roles_view($arrAttr)
	{
	...
	Function PageLogic()
	{
		...
		if ($this->Post('add')){
			$this->SendTo('rbac_users_has_roles_add.php');
		}
		else if ($this->Post('edit')){
			$this->SendTo('rbac_users_has_roles_edit.php', array('id'=>$_id));
		}
	...
	Function ProcessPage(){
		...
		$arr_sql = array(
			...
			'from'=>array('rbac_users_has_roles as t1'),
			'join'=>array(
				'INNER JOIN rbac_roles as t2 ON t2.id = t1.roles_id',
				'LEFT JOIN users as t3 ON t3.id = t1.users_id'
				),
			'where'=>'',
			'order'=>'username',
			'group'=>'',
		...
		$_tabulate->SetTitle('users has roles'.':');
