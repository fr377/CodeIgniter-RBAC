...
class rbac_privileges_view extends securePage_bv
{

	Function rbac_privileges_view($arrAttr)
	{
	...
	Function PageLogic()
	{
		...
		if ($this->Post('add')){
			$this->SendTo('rbac_privileges_add.php');
		}
		else if ($this->Post('edit') && isset($_id)){
			$this->SendTo('rbac_privileges_edit.php', array('id'=>$_id));
		}
		else if ($this->Post('add_actions') && isset($_id)){
			$this->SendTo('rbac_privileges_add_actions.php', array('id'=>$_id));
		}
		else if ($this->Post('delete') && isset($_id)){
			include_once CLASSES_DIR.'rbac/class.rbacAdmin_bv.php';
			
			$_sql = "SELECT name FROM rbac_privileges WHERE id = $_id";
			$_row = $this->mConn->GetRow($_sql);
			
			rbacAdmin_bv::DeletePrivilege($_row['name'], $this->mConn);
		}
	...
	Function ProcessPage(){
		...
		$_tabulate->SetButton('add', 'New');
		$_tabulate->SetButton('edit', 'View / Edit');
		$_tabulate->SetButton('add_actions', 'Add actions');
