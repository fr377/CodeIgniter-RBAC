<?php
/**
  * Editing functionality for table rbac_users_has_roles
  *
  **/
...
class rbac_users_has_roles_edit extends securePage_bv
{

	Function rbac_users_has_roles_edit($arrAttr)
	{
		...
		include_once FORM_DIR.'frm.rbac_users_has_roles.php';
		
		$_db = $this->mConn;
		$_db->SetTable('rbac_users_has_roles');
		
		if ($_frm->Validate($this->Post())){
			
			$_db->StoreData($_frm->GetDbData('rbac_users_has_roles'));
		...
		$this->Assign('<TITLE>', 'Update rbac_users_has_roles');
