...
class rbac_actions_view extends securePage_bv
{

	Function rbac_actions_view($arrAttr)
	{
	...
	Function PageLogic()
	{
		...
		if ($this->Post('add')){
			$this->SendTo('rbac_actions_add.php');
		}
		else if ($this->Post('edit') && isset($_id)){
			$this->SendTo('rbac_actions_edit.php', array('id'=>$_id));
		}
		else if ($this->Post('delete') && isset($_id)){
			include_once CLASSES_DIR.'rbac/class.rbacAdmin_bv.php';
			
			$_sql = "SELECT name FROM rbac_actions WHERE id = $_id";
			$_row = $this->mConn->GetRow($_sql);
			
			rbacAdmin_bv::DeleteAction($_row['name'], $this->mConn);
		}
